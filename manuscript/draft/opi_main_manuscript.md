...
Figure 3 presents the per-window OPI time series for the three task hypotheses computed from the 128-minute recording. Under the CAT II ILS approach hypothesis (w1 = 0.55, Task_mod = 0.90), the composite varied between approximately 64 and 72 across the recording and was classified predominantly in the **CAUTION** band (55-69) with brief transitions to **GO (Monitor)** (70-84) during windows of higher HRV_recovery and lower stress index. Under the UAS ISR hypothesis (w1 = 0.35, w2 = 0.30, w3 = 0.15, w4 = 0.20, Vigilance_adj evaluated at the relevant time-on-task), the composite was lower overall and drifted downward with simulated time-on-task, consistent with the Warm vigilance decrement and reaching the **CAUTION** band by the two-hour mark. Under the carrier-landing hypothesis (w1 = 0.50, w2 = 0.30, w3 = 0.20), the composite was most sensitive to short-term HRV recovery and produced the widest variability across windows, with intermittent transitions between **CAUTION** and **GO (Monitor)** bands.

The central observation is that identical physiological input yielded substantively different composite outputs across the three task hypotheses, consistent with the MRT-derived weight-profile design intent. This demonstrates that the framework exposes task-specific readiness information that a task-agnostic composite would collapse. The numerical outputs are illustrative only; they do not reflect the readiness state of any operator performing any of the three tasks, because the recording was collected from a single individual in a non-operational context. Table 2 supplies the numerical worked example for the CAT II ILS approach under a representative window.

### 3.2 Engineering verification of the OPI fusion pathway

Engineering verification of the reference implementation covers the major paths through the OPI pipeline at the software level. Automated tests exercise scheduling-core readiness fusion (`tests/test_scheduling_core.py`), fatigue-risk-management behaviour and SAFTE-adjacent logic (`tests/test_frms.py`, `tests/test_frms_v2.py`, `tests/test_fatigue_integration.py`), API endpoint normalisation and behaviour (`tests/test_api_user_profile_normalization.py`, `tests/test_research_windowed_endpoint.py`), environmental-modifier ingestion, caching, and alignment (`tests/test_noaa_cache.py`, `tests/test_space_weather_impact.py`, `tests/test_space_weather_alignment.py`), and broader statistical and charting modules (`tests/test_comprehensive_modules.py`). A coverage map at the OPI-component level is supplied in Table 5.

These tests justify reporting that the readiness fusion, orchestration, and environmental-modifier layers are implemented and regression-tested at the software level. They do not establish operational validation, clinical deployment readiness, or numerical equivalence of the HRV engine to external reference packages. In particular, the windowed-endpoint test monkeypatches the inner HRV computation to isolate endpoint behaviour, so that pathway is verified more directly than the underlying numerical implementation.
