...
### Integration goals
1) Preserve validated schedule‑based structure (sleep/circadian/inertia) for out‑of‑sample stability. 2) Add physiology‑based acute‑state sensitivity without overfitting. 3) Embed personalization and rigorous error control (mixed‑effects, blocked CV, FDR). 4) Provide transparent reason codes for decisions and enable FRMS‑style auditability.
...
– Folkard, S., Robertson, K. A., & Spencer, M. B. (2007). A fatigue/risk index to assess work schedules. Somnologie, 11, 177–185.  
...
– McCauley, P., et al. (2009). A new mathematical model for the homeostatic effects of sleep loss on neurobehavioral performance. Journal of Theoretical Biology, 256, 227–239.  
...
Notes: UMP/2B‑Alert caffeine‑optimization work has multiple publications; see recent SLEEP reports summarizing unified modeling and personalized caffeine dosing algorithms. HSE FRI and BAM are also documented in agency/vendor sources when DOIs are unavailable.
