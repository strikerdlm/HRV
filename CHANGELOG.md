...
### Changed
- **Unified Space Data tab (data-only, no correlations)** (`app/app.py`): Merged the former **🌍 Space Weather** (SWPC + DONKI) and **🛰️ NOAA Space** dashboards into a single **🌐 Space Data** tab that fetches and visualizes external datasets from the sources, but intentionally **removes/disables all correlation + ML workflows** to keep this tab stable and independent from HRV processing.
