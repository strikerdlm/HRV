...
### Added
- **Manuscript — Antarctic aerial-campaign validation narrative** (`manuscript/draft/opi_main_manuscript.md`, `manuscript/evidence/evidence_matrix.md`, `manuscript/evidence/validation_story.md`, `analysis/build_figure{1,2,4}.py`):
  - Abstract Conclusions, §1.4 Objective, §2.5 Reference implementation, §4.4 Strengths, §4.5 Limitations, §4.6 Validation roadmap, and §6.6 Acknowledgments reframed around a planned Colombian Air Force C-130 Hercules Antarctic aerial campaign (multi-leg Drake Passage sorties during the 2026–2027 austral summer) using ActiGraph wGT3X-BT wrist accelerometry paired with Polar H10 chest-strap electrocardiography.
  - §2.5 expanded to document three inspectable reference-implementation modules (PVT, actigraphy-backed sleep, longitudinal trajectory-risk aligned with McEwen allostatic load) replacing the earlier two-module framing.
  - Experimental NOAA space-weather module retired from the manuscript narrative; test-file references removed from §3.2, environmental-modifier acknowledgement removed from §6.6.
  - "Garmin-backed sleep module" language replaced with "actigraphy-backed sleep module"; sleep-analytics §4.5 limitation reframed as operational-mode bound with research-grade ActiGraph wGT3X-BT pathway for validation.
  - Figures 1, 2, 4 redesigned with coordinated muted publication-grade palette; Figure 4 no longer depicts Streamlit, Garmin, or NOAA components and foregrounds ActiGraph + Polar H10 research-grade instrumentation at L1.
  - References list gains four DOI-verified citations: Buchan 2024 (DOI 10.1123/jmpb.2023-0026), Hinde/White/Armstrong 2021 (DOI 10.3390/s21041061), Pereira et al. 2020 (DOI 10.1123/ijspp.2019-0406), Yang & Ben-Menachem 2024 (DOI 10.1007/s10877-023-01080-8).

- **Sleep module (Garmin-backed)** (`app/sleep_core.py`, `tests/test_sleep_core.py`, `api/research_endpoints.py`, `frontend/src/lib/sleep-metrics.ts`, `frontend/src/components/sleep/sleep-charts.tsx`, `frontend/src/app/research/sleep/page.tsx`, `frontend/src/app/scheduling/sleep/page.tsx`, `docs/SLEEP.md`):
  - Canonical Python scoring in `app/sleep_core.py` with 27 pytest tests (all pass): sleep debt (7-night rolling deficit vs 7.5 h target), Sleep Regularity Index per Lunsford-Avery 2018 (DOI 10.1038/s41598-018-32402-5) via 5-minute epoch matching between consecutive 24-hour cycles, bedtime / waketime / midpoint standard deviations, stage-balance derivations (deep+REM proportion, deep:REM ratio), low-SpO₂ screening proxy with four-band scoring (NORMAL / MILD / ELEVATED / HIGH — never AHI or apnea diagnosis), and operational GO / GO_MONITOR / CAUTION / NO_GO readiness gate.
  - Pearson and Spearman correlation engine with two-sided p and Benjamini-Hochberg FDR-adjusted q across the Pending.md §4 Tier A pair set (sleep duration / score / efficiency / deep+REM / RHR / SpO₂ / respiration × HRV RMSSD); SciPy-backed when available, pure-Python fallback otherwise; underpowered flag when n_nights < 14 per Pending.md §5.
  - Three FastAPI endpoints added to `api/research_endpoints.py` under the existing `/api/research/garmin` prefix: `/sleep-summary/{user_id}`, `/sleep-correlations/{user_id}`, `/sleep-debt-trend/{user_id}`. All read from the shared `garmin_daily_metrics` table via `UserDatabase.get_garmin_daily_metrics`.
  - TypeScript layer (`frontend/src/lib/sleep-metrics.ts`) mirrors `SLEEP_THRESHOLDS` constants + response shapes; does not duplicate scoring.
  - Seven reusable ECharts-based sleep visualisations (`frontend/src/components/sleep/sleep-charts.tsx`): nightly duration trend with 7-9h target band + 5h hard-floor line, 7-night rolling debt curve with CAUTION/NO-GO markLines, stage-balance stacked bar, latest-night stage pie, correlation matrix heatmap with FDR-q tooltips, pair scatter with OLS fit, regularity bedtime / waketime strip. All follow `.cursor/rules/plots.mdc` (SCIENTIFIC_COLORS, clean grid margins, single-line titles).
  - Research dashboard at `/research/sleep` — headline KPIs, operational readiness banner, duration and debt trends, stage balance, regularity strip, correlation matrix, six per-pair scatter plots (duration / score / deep / REM / SpO₂ / respiration × HRV RMSSD), evidence map card with tooltipped PubMed / DOI chips.
  - Operational gate at `/scheduling/sleep` — four-band decision banner, KPIs colour-coded to gate thresholds, debt gauge with CAUTION/NO-GO segmentation, last-7-night duration bar with accept/floor markLines, SpO₂ screening panel, 30-night debt curve, decision-detail table, explicit Garmin-vs-PSG disclosure card citing Lee 2025 (DOI 10.5664/jcsm.11460) and Schyvens 2024 (DOI 10.2196/52192).
  - Scope discipline: Tier A only. Tier B (hypnograms / arousals / fragmentation / WASO / latency) and Tier C (apnea diagnosis, AHI, RDI) are explicitly out of scope. UI never uses clinical sleep-disorder classification language; all SpO₂ content is labelled "screening only".
  - Platform documentation at `docs/SLEEP.md` with nine DOI-verified validation references (Lunsford-Avery 2018, Lee 2025, Schyvens 2024, Liao 2010, Kesek 2009, Zhang 2023, Zhu 2023, Balali 2025, Task Force 1996).
...
### Documentation
- **OPI manuscript — novelty and principled-simplicity argument sharpened** (`manuscript/draft/opi_main_manuscript.md`):
  - Introduction §1.4 extended from four to five explicit contributions. The new fifth contribution makes the epistemic case explicit: the OPI is deliberately simple as a design choice, inspectable by safety reviewers, extensible through demand analysis rather than training, calibrable on modest operational samples via Bayesian hierarchical or penalised-regression methods, and falsifiable at the level of individual per-task weight profiles. The simplicity is framed as principled in aerospace contexts where labelled outcome data are scarce, populations are small, sensor generations turn over faster than training cycles, and the cost of unexplainable NO-GO decisions is large.
  - Discussion §4.1 opening rewritten to foreground the central claim: "neither a new physiological marker nor a new machine-learning classifier but a deliberately simple, theoretically grounded substrate". Operational trustworthiness is positioned as the design target, not in-distribution predictive accuracy.
  - Discussion §4.3 renamed "Theoretical grounding and the case for principled simplicity" and expanded from two paragraphs to five: (a) the three theoretical commitments that structure the composite (MRT, allostatic load, situation awareness); (b) the four structural constraints against a classifier-first strategy in aerospace (labelled-data scarcity, population heterogeneity and transfer failure, sensor-generation drift, asymmetric-cost/safety-review context); (c) the four positive properties that follow from a linear theory-derived composite (inspectability, extensibility, calibrability at modest n, Popperian falsifiability at the per-weight-profile level); (d) the hybrid-classifier pathway positioning classifiers as local refinements of the substrate rather than replacements; (e) Occam's razor in the operational-science sense.
  - Word count 7,748 — remains within the 8,000 Applied Ergonomics methodology-paper budget including references.
- **OPI manuscript hard-analysis refresh** (`manuscript/draft/opi_main_manuscript.md`, `manuscript/evidence/evidence_matrix.md`, `manuscript/README.md`, `frontend/src/app/about/page.tsx`, `frontend/src/lib/version.ts`):
  - Methods §2.5 now explicitly describes the in-platform PVT module (three validated variants, browser and PsychoPy desktop surfaces, feeds `pvt_lapses_3min` into the readiness-fusion hard gate) and the Garmin-backed sleep module (sleep debt, Sleep Regularity Index, stage balance, low-SpO₂ screening proxy, four-band operational sleep gate) as inspectable reference-implementation modules rather than external dependencies.
  - Results §3.2 engineering-verification paragraph updated to explicitly list the 28 PVT-module tests and 27 sleep-module tests; test surface now covers readiness fusion, orchestration, environmental modifier, vigilance, and sleep layers at the software level.
  - Discussion §4.4 strengths expanded to emphasise that the delivered PVT and sleep modules concretise the composite's vigilance and fatigue inputs as inspectable, regression-tested code.
  - Discussion §4.5 adds a seventh limitation (Garmin sleep vs PSG bounds per Lee 2025 and Schyvens 2024; screening-only SpO₂ posture) and an eighth limitation (browser PVT 5-10 ms RT precision per Anwyl-Irvine 2020 vs PsychoPy sub-ms desktop option per Garaizar & Vadillo 2014).
  - Discussion §4.6 validation roadmap notes that the in-platform PVT and sleep modules now expose concrete daily-collectable feeds (`pvt_lapses_3min`, `sleep_regularity_index`, `cumulative_debt_hours_7d`) and the FDR-adjusted correlation engine for the proposed per-task weight-calibration study.
  - References expanded with eight new Crossref-verified DOIs: Basner & Dinges 2011, Dinges et al. 1997, Grant et al. 2017, Anwyl-Irvine et al. 2020, Garaizar & Vadillo 2014, Lunsford-Avery et al. 2018, Lee et al. 2025, Schyvens et al. 2024.
  - Evidence matrix (`manuscript/evidence/evidence_matrix.md`) adds four new Supported claims for the PVT module, the sleep module, and the correlation engine.
  - About page (`frontend/src/app/about/page.tsx`) operational capabilities extended with Operational Sleep Gate, Research Sleep Dashboard, and Research PVT cards; references list expanded with the six new validation anchors. Version constants bumped to 1.18.0-opi-pvt-sleep / 2026-04-21.
...
- **Rendered manuscript figures** (`manuscript/figures/*.svg`, `manuscript/figures/figure_plan.md`): Added SVG figure assets for platform architecture, translational workflow, research-to-operations coupling, and verification coverage, then updated planning files with asset names, sizing, and insertion points.
- **Bibliography and standards verification for manuscript** (`manuscript/draft/main_manuscript_scaffold.md`, `manuscript/references/seed_references.md`, `manuscript/references/mcp_research_notes.md`): Verified missing DOI metadata, corrected incomplete reference entries, added official NASA/ICAO standards citations, and logged the MCP-backed reference-verification pass used to tighten the submission-candidate draft.
- **Supplementary manuscript appendix** (`manuscript/supplement/submission_support_appendix.md`, `manuscript/supplement/supplement_outline.md`): Added a consolidated reviewer-facing appendix for extended engineering verification, standards crosswalks, reporting-guideline positioning, non-claims, and deployment prerequisites, then linked it from the manuscript draft.
- **Node-first manuscript rewrite** (`manuscript/draft/main_manuscript_scaffold.md`, manuscript tables/figures/evidence docs): Reframed the paper around a Next.js/Node.js-first client over FastAPI and a shared Python modeling backend, deepened the biomathematical treatment of HRV, SAFTE/circadian dynamics, readiness fusion, and environmental timing, added a model-layer table, and synchronized the figures and evidence package with the new scope.
- **Manuscript agent orientation** (`manuscript/README.md`): Expanded the manuscript folder README with pipeline status, Q1 journal roadmap, guardrails, and read-order guidance so agents know what is complete, what blocks submission, and which files anchor claims.
